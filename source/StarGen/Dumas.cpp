...
//
{
    ...
    i = 0;  //MPS: replaces bizarre statement above
}
#endif
#endif
}
...
static int dumasCheckSegment (dumasDIf *segment, char *moduleName, long lineNo, int dumping)
{
	...
	{
		BOOM ((char*)"The Dumas header has been undermined.");
		retval = FALSE;
	}
	else
	{
		...
		{
			BOOM ((char*)"The Dumas header has been overwritten.");
			retval = FALSE;
		}
		else
		{
			...
			{
				BOOM ((char*)"Item is poisoned.");
				retval = FALSE;
			}
				
			if (dumping && deserted == segment->length)
			{
				BOOM ((char*)"Item was never used.");
				retval = FALSE;
			}
			...
			for (i = 0; i < 4; i++)					// Check 4 bytes of guard longword
			{
				if (*cptr++ != (char) ENDING)
				{
					BOOM ((char*)"The data has been overrun.");
					retval = FALSE;
					break;
				}
			}
...
static void *dumasAlloc (long requestedBytes, char *moduleName, long lineNo)
{
	...
	if (DUMAS_ACTIVE & dumasFlags)
	{
		...
		if (0 == requestedBytes)
			dumasComplain ((char*)"Zero bytes requested.", callerName, lineNo, NULL, 0);
...
static void dumasCheckAndFree (void *oldPtr, char *moduleName, long lineNo)
{
	...
	if (DUMAS_ACTIVE & dumasFlags)
	{
		...
		if (segment->guard != MIDDLE)
		{
			BOOM ((char*)"The Dumas data has been overwritten.");
			return;
		}
		...
		for (i = 0; i < 4; i++)					// Check 4 bytes of wall longword
		{
			if (*cptr++ != (char) ENDING)
			{
				BOOM ((char*)"The data has overrun.");
				break;
			}
		}
		...
		for (sptr = monteCristoHoard; sptr && sptr != segment; sptr = sptr->next)
		{
			...
			{
				BOOM ((char*)"Too many things on the allocated list.");
				sptr = NULL;
			}
			lastSegment = sptr;
		}
		...
		if (NULL == sptr)
		{
			BOOM ((char*)"Freed something not on the allocated list.");
			//
