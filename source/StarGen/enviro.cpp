...
int orb_zone(long double luminosity, long double orb_radius)
{
    ...
    if (orb_radius < (0.8 * sqrt(luminosity)))
		return(1);
	//else if (orb_radius < (15.0 * sqrt(luminosity)))
    else if (orb_radius < (1.4 * sqrt(luminosity)))
		return(2);
	else 
		return(3);
}
...
//int giant, zone;
{
	...
	temp2 = temp2 / (A1_20 * pow(atomic_num,2));
	temp2 = 1.0 + temp2;
	temp = temp / temp2;
							  ...
							  long double r_ecosphere, int gas_giant)
{
	...
	temp = temp * pow(r_ecosphere / orb_radius, 1./4.);
	if (gas_giant)
		return(temp * 1.2);
	else 
		return(temp * 5.5);
...
//long double mass, equat_radius;
{
	...
	volume = (4.0 * PI * pow(equat_radius,3)) / 3.0;
	return(mass / volume);
}
...
//long double separation, small_mass, large_mass;
{
	...
	period_in_years = sqrt(pow(separation,3) / (small_mass + large_mass));
...
long double day_length(planet_pointer	planet)
{
	...
	base_angular_velocity = sqrt(2.0 * J_STARGEN * (planetary_mass_in_grams) /
								 (k2 * pow(equatorial_radius_in_cm,2)));
...
int inclination(long double orb_radius)
...
//long double mass, equat_radius, exospheric_temp;
{
	...
	return ((3.0 * MOLAR_GAS_CONST * exospheric_temp) /
			(pow((esc_velocity/ GAS_RETENTION_THRESHOLD) / CM_PER_METER,2)));
...
//long double volatile_gas_inventory, planet_radius;
{
	...
	temp = (0.71 * volatile_gas_inventory / 1000.0)
			 * pow(KM_EARTH_RADIUS / planet_radius,2);
	if (temp >= 1.0)
		return(1.0);
	else 
		return(temp);
...
//	hydro_fraction;
{
	...
	{
		surf_area = 4.0 * PI * pow(equat_radius,2);
...
//long double ecosphere_radius, orb_radius, albedo;
{
	return(sqrt(ecosphere_radius / orb_radius)
		  * pow((1.0 - albedo) / (1.0 - EARTH_ALBEDO), 1./4.)
...
//long double optical_depth, effective_temp, surf_pressure;
{
	...
	long double rise = (pow(1.0 + 0.75 * optical_depth, 1./4.) - 1.0) *
					 ...
					 planet_pointer planet)
{
	...
	long double t = (pow(v,3) / (2.0 * pow(g,2) * r)) * exp((3.0 * g * r) / pow(v,2));
	long double years = t / (SECONDS_PER_HOUR * 24.0 * DAYS_IN_A_YEAR);
	
//	long double ve = planet->esc_velocity;
//	long double k = 2;
//	long double t2 = ((k * pow(v,3) * r) / pow(ve,4)) * exp((3.0 * pow(ve,2)) / (2.0 * pow(v,2)));
