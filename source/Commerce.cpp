...
void Ship::setDestination(NavObj *dest) {
    Destination = dest;
}
...
stdstring Facility::getTypeName() {
    return TypeName;
}
...
void IndustrialCenter::addFacility(Facility f) {
    Facilities.push_back(std::move(f));
}

void IndustrialCenter::removeFacility(int i) {
    Facilities.erase(Facilities.begin()+i);
}

Facility* IndustrialCenter::getFacility(int i) {
    return &Facilities.at(i);
}
...
Station::Station(AstroObj* primary)
: PublicBay(std::numeric_limits<int>::max())
{
    Primary = primary;
}

bool Station::checkIfDocked(Ship_ptr s) {
    for (int i=0; i<Hangar.size(); i++) {
        if (s==Hangar[i]) return true;
    }
    return false;
}

void Station::dock(Ship_ptr s) {
    if (!checkIfDocked(s)) {
        Hangar.push_back(s);
    }
}

void Station::undock(int i) {
    Hangar.erase(Hangar.begin()+i);
}

void Station::undock(Ship_ptr s) {
    for (int i=0; i<Hangar.size(); i++) {
        if (s==Hangar[i]) {
            undock(i);
            break;
